...
地址写法与 TCP 完全相同，协议名用 `modbus-rtu`，设备的 `host` 字段填**串口名**
（Linux 上形如 `/dev/ttyUSB0`，Windows 上形如 `COM3`），
波特率等参数放 `extra`：`baudRate` / `parity` / `dataBits` / `stopBits`。

**一条 RS-485 总线要配成一台 Rung 设备，不是多台。** 总线上的多个从站用地址
前缀区分（`3:HR100`）——串口是独占资源，配成多台设备会让多个采集工作者去抢
同一个串口，谁也打不开。这一点和 TCP 完全相反，是从 TCP 迁过来时最容易配错的地方。
...
- **失败和被拒绝的尝试同样留痕**。只记成功的审计，等于把"谁试图动了什么但没成"丢掉了。
  写到设备失败、点位只读、点位不存在、密钥缺失或无写权限——四类都会落一条
...
- **写审计**：谁、什么时候、往哪个点位写了什么，以及设备回读到的实际值。
  被拒绝的尝试同样列出来——「谁试图往只读点位写东西」「谁没带密钥就来写」
  是安全审计里最该看到的信号
...
**还有一对内存串口**，把 FluentModbus 的 RTU 客户端和服务端直接对接起来：
走的是真实的 RTU 帧、含 CRC 校验，只有物理层换成了内存。
串口设备进不了 CI，虚拟串口工具也不是每台机器都有，这样帧格式、CRC、
从站寻址、多从站共线全都能自动化验证。**测不到的**是波特率、校验位、
RS-485 收发切换时序和线缆干扰——那些只有真串口能验。
